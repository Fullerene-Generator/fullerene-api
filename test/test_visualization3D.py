...
from .testconstants import C30_INPUT, C30_3D_EMBEDDER_OUTPUT, C30_EDGES_EMBEDDING, C30_3D_EMBEDDING_OUTPUT, EXCEPTION_REASON
...
class Test2DVisualization(BaseIntegrationTest):
    @pytest.mark.asyncio
    @patch('app.core.generator.asyncio.create_subprocess_exec')
    @patch('app.api.fullerene_data.asyncio.create_subprocess_exec')
    async def test_whenFullerneGenerated_shouldReturn3Dcoords(self, mock_embedder_subprocess_exec, mock_create_subprocess_exec, sqlite_cache):
    ...
    @patch('app.core.generator.asyncio.create_subprocess_exec')
    @patch('app.api.fullerene_data.asyncio.create_subprocess_exec')
    async def test_whenGetFullereneFails_shouldReturnRelevantException(self, mock_embedder_subprocess_exec, mock_create_subprocess_exec, sqlite_cache):
    ...
    @pytest.mark.asyncio
    @patch('app.api.fullerene_data.asyncio.create_subprocess_exec')
    async def test_whenFullerneNotGenerated_shouldReturn404NotFound(self, mock_embedder_subprocess_exec, sqlite_cache):
    ...
    @patch('app.core.generator.asyncio.create_subprocess_exec')
    @patch('app.api.fullerene_data.asyncio.create_subprocess_exec')
    async def test_when3DEmbedderRespondsWithError_shouldRaiseRelevantException(self, mock_embedder_subprocess_exec, mock_create_subprocess_exec, sqlite_cache):
        ...
        assert response.status_code == 500
        assert response.json() == {
            "detail": f"Error running 3D embedder due to : {EXCEPTION_REASON}"
        }
        app.dependency_overrides = {}